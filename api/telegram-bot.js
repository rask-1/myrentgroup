...
export default async function handler(req, res) {
  ...
  if (callback) {
    ...
    if (data === 'lang_ru' || data === 'lang_en') {
      ...
      global.botStates[userId] = { ...state, lang: newLang, step: 'privacy' };
    ...
    if (data === 'privacy_agree') {
      global.botStates[userId] = { ...state, step: 'privacy_agreed', privacyAccepted: true };
      await api('sendMessage', {
        chat_id: chatId,
        text: t.mainMenu,
        reply_markup: { inline_keyboard: [[
    ...
    if (data === 'privacy_decline') {
      delete global.botStates[userId];
      await api('sendMessage', { chat_id: chatId, text: t.cancel });
      return res.status(200).json({ ok: true });
    }
  ...
  if (message?.reply_to_message && state.step) {
    if (state.step === 'waiting_name' && text) {
      global.botStates[userId] = { ...state, step: 'waiting_phone', name: text.trim() };
      await api('sendMessage', {
        chat_id: chatId,
        text: t.enterPhone,
    ...
    if (state.step === 'waiting_phone' && text) {
      ...
      try {
        await fetch('https://myrentgroup.com/api/lead', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: name,
            phone: phone,
            formType: 'telegram_bot',
            source: 'Telegram Bot',
            language: lang,
        ...
        await api('sendMessage', { chat_id: chatId, text: t.error });
      }
